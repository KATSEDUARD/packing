document.addEventListener('DOMContentLoaded', function (event) {
    ...
    function write_tank_dimensions() {
        ...
        if (BinPackerPrepareModules.tank.build(tank_w, tank_h) && !(tank_w === '' || tank_h === '')) {
            console.log(BinPackerPrepareModules.tank);
            message.innerHTML = '';
            container_dimensions.innerText = ' ' + tank_w + ' x ' + tank_h;
        } else {
            message.style.color = 'red';
            message.innerHTML = 'Введіть коректні значення';
        }
    ...
    function write_boxes_dimensions() {
        ...
        if (BinPackerPrepareModules.boxes.build(box_name, box_w, box_h)) {
            ...
            add_to_displayed_list_of_boxes(
                box_name,
                box_w,
                box_h,
                BinPackerPrepareModules.boxes.tmpSet.length
            );
            activate_prepare();
        }
    }
    ...
    function show_list_of_boxes() {
        ...
        for (
            let i = 0, len = BinPackerPrepareModules.boxes.tmpSet.length;
            i < len;
            i++
        ) {
            console.log(BinPackerPrepareModules.boxes);
    ...
    function add_to_displayed_list_of_boxes(
        box_name,
        box_w,
        box_h,
        alternate_name
    ...
    function show_hide_input() {
        ...
        if (
            (remember_last_node.length == 1 &&
                this.id != remember_last_node[0].node.id) ||
            (remember_last_node.length > 1 &&
                this.id != remember_last_node[1].node.id)
        ) {
            remember_last_node[0].node.addEventListener(
                'click',
                show_hide_input
            );
            if (
                remember_last_node.push(tmpObj) &&
                remember_last_node.length > 2
            ) {
        ...
        this.innerHTML =
            '<input id="edit_box_name" name="b_name" value="' +
            default_values.name +
            '">' +
            '<input type="number" id="edit_box_h" name="box_h" value="' +
            default_values.height +
            '">' +
            '<input type="number" id="edit_box_w" name="box_w" value="' +
            default_values.width +
            '">' +
            ' <button class="btn btn-danger" id="delete_box_' +
            i +
            '" name="delete_box" data-del-box="' +
            i +
            '">Видалити</button>' +
            ' <button class="btn btn-success" id="save_box_' +
            i +
            '" name="save_box" data-save-box="' +
            i +
            '">Зберегти</button>';
        ...
        if (remember_last_node.length > 1) {
            ...
            console.log(
                remember_last_node[0].node.id +
                    ' ' +
                    remember_last_node[1].node.id
            );
        }
        ...
        function remove_box() {
            ...
            remember_last_node[0].node.removeEventListener(
                'click',
                show_hide_input,
                true
            );
        ...
        function save_box() {
            ...
            if (
                ...
                BinPackerPrepareModules.boxes.tmpSet[box_id].width =
                    Number(width);
                BinPackerPrepareModules.boxes.tmpSet[box_id].height =
                    Number(height);
                BinPackerPrepareModules.boxes.tmpSet[box_id].name = name;
                message.innerHTML = '';
            } else {
                message.style.color = 'red';
                message.innerHTML = 'Введені не числові значення!';
            }
        ...
        let previous_del_btn = document.getElementById(
            'delete_box_' + remember_last_node[0].node.id
        );
    ...
    function activate_prepare() {
        if (
            BinPackerPrepareModules.boxes.tmpSet.length > 0 &&
            number_validation(
                BinPackerPrepareModules.tank.width,
                BinPackerPrepareModules.tank.height
            )
        ) {
    ...
    function show_list_of_oversized_boxes() {
        ...
        for (
            let i = 0, len = BinPackerPrepareModules.list_oversized.length;
            i < len;
            i++
        ) {
            let li = document.createElement('li');
...
var BinPackerPrepareModules = (function () {
    ...
    var boxes = {
        ...
        build: function (name, width, height) {
            ...
            try {
                if (
                    number_validation(width, height) &&
                    width.length > 0 &&
                    height.length > 0
                ) {
                    if (!name) {
                        name = this.tmpSet.length + 1;
                    }
                    this.tmpSet.push({
                        name: name,
                        width: Number(width),
                    ...
                    throw 'Введені не числові значення!';
                }
            } catch (err) {
                message.style.color = 'red';
    ...
    var oversized = {
        ...
        testAndSaveOversized: function (value) {
            if (
                that.getOversized(value, tank) ||
                that.getUndersized(value, tank)
            ) {
        ...
        getOversized: function (value, tank) {
            if (
                (value.width > tank.width && value.width > tank.height) ||
                (value.height > tank.width && value.height > tank.height)
            ) {
                return true;
            } else {
                return false;
...
var BinPackerInsertModules = (function () {
    ...
    function ContactPointScoreNode(x, y, width, height) {
        ...
        if (x == 0 || x + width == container.width) score += height;
        if (y == 0 || y + height == container.height) score += width;

        for (var i = 0, tot = usedBoxes.length; i < tot; i++) {
            if (
                usedBoxes[i].x == x + width ||
                usedBoxes[i].x + usedBoxes[i].width == x
            )
                score += CommonIntervalLength(
                    usedBoxes[i].y,
                    usedBoxes[i].y + usedBoxes[i].height,
                    y,
                    y + height
                );
            if (
                usedBoxes[i].y == y + height ||
                usedBoxes[i].y + usedBoxes[i].height == y
            )
                score += CommonIntervalLength(
                    usedBoxes[i].x,
                    usedBoxes[i].x + usedBoxes[i].width,
                    x,
                    x + width
                );
        }
    ...
    function FindPositionForNewNodeBottomLeft(width, height, name) {
        ...
        for (var i = 0, tot = freeBoxes.length; i < tot; i++) {
            if (freeBoxes[i].width >= width && freeBoxes[i].height >= height) {
                ...
                if (
                    topSideY < bestY ||
                    (topSideY == bestY && freeBoxes[i].x < bestX)
                ) {
                    bestNode.x = freeBoxes[i].x;
                    bestNode.y = freeBoxes[i].y;
                    bestNode.width = width;
    ...
    function FindPositionForNewNodeBestAreaFit(width, height, name) {
        ...
        for (var i = 0, tot = freeBoxes.length; i < tot; i++) {
            var areaFit =
                freeBoxes[i].width * freeBoxes[i].height - width * height;
            ...
            if (freeBoxes[i].width >= height && freeBoxes[i].height >= width) {
                var flippedLeftoverHoriz = Math.abs(
                    freeBoxes[i].width - height
                );
                var flippedLeftoverVert = Math.abs(freeBoxes[i].height - width);
                var flippedShortSideFit = Math.min(
    ...
    function FindPositionForNewNodeBestLongSideFit(width, height, name) {
        ...
        for (var i = 0, tot = freeBoxes.length; i < tot; i++) {
            ...
            if (freeBoxes[i].width >= height && freeBoxes[i].height >= width) {
                ...
                var flippedLongSideFit = Math.max(
                    flippedLeftoverHoriz,
                    flippedLeftoverVert
                );
...
function insert_boxes() {
    for (var i = 0; i < insertMethods.length; i++) {
        ...
        cargo[i] = {
            insertMethod: insertMethods[i],
            usedBoxes: usedBoxes.slice(0),
...
function show_select() {
    ...
    for (let i = 0; i < insertMethods.length; i++) {
        html_output +=
            '<option value="' + i + '">' + insertMethods[i] + '</option>';
    }
